...
import { AlertTriangle, Droplets, Map, Zap } from 'lucide-react';
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setAlerts(
          alertsResponse.data.filter((alert) => !alert.isResolved),
        );
      } catch {
        setError(
  ...
  const bounds = useMemo(
    () =>
      metersWithCoordinates.map(
        (meter) =>
          [meter.latitude, meter.longitude] as [number, number],
      ),
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-400">
          {meters.length} compteurs · {alerts.length} alerte(s) active(s)
        </p>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="overflow-hidden rounded-2xl border border-white/10 lg:col-span-2">
            ...
            >
              ...
              {metersWithCoordinates.map((meter) => {
                const isWater = meter.type === 'WATER';
                const hasAlert = activeAlertMeterIds.has(meter.id);
                    ...
                    pathOptions={{
                      color: hasAlert
                        ...
                        : isWater
                          ? '#22d3ee'
                          : '#fbbf24',
                      fillColor: isWater ? '#06b6d4' : '#f59e0b',
                      fillOpacity: 0.85,
                      weight: hasAlert ? 4 : 2,
                    }}
                    eventHandlers={{
                    ...
                    <Popup>
                      <div className="min-w-[190px]">
                        ...
                        <p className="mb-1 text-sm text-slate-700">
                          Type :{' '}
                          {isWater ? 'Compteur d’eau' : 'Compteur électrique'}
                        </p>
          ...
          <div className="space-y-3">
            {selectedMeter ? (
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-md">
                <p className="text-sm text-slate-400">
                  Cliquez sur un compteur pour afficher ses détails.
                </p>
              </div>
            )}
